...
\begin{frame}
	...
	\item<2-> The largest population of Native American
	  % officiers have the possibility to recruit enough
          % navajo for the program

	  % Then, as Eric mentionned, we can qualified the language
          % as obscure because just a handful of non-Navajo were able
          % to speak it. Mostly, due to the fact, it was not written.
	...
	\item<3-> It remained mostly "unwritten".
	  ...
	  % But, we can also describe the code as secure because the grammar
          % is complex, very different to German and Japanesse
	...
	\item<4-> Imposture aren't easy to make
          % And, above all it is difficult to pretend to be someone else,
          % it require a good accent.
    \end{itemize}
\end{frame}
% ------------------------------------------------------------------------------------------------
...
\begin{frame}
    ...
    % Now, if you remember Eric said that in 1942, 29 Navajos were recuit
    % to create a code
    % The reason is that in navajo language,
    % there is a lack of military terms
    % so, they needed to created a dictionnary for common
    % military words
    Because of a lack of military terms, there was the need
...
\begin{frame}
    ...
    % Then, they build a phonectic alphabet where one lettre represent a word they choose.
    % After, this work the word is translated in the language
    \begin{itemize}
...
% At the end, on the creation, they have written 17 pages of code they needed to memorize.
% It was easy for them beacause of their oral culture. They have the habit to memorize
% their song, their prayer
...
\begin{frame}
  ...
  % Unfortunately, they realised that the code was not enough rich. So, they expand it
  % with military terms they passed from 211 to 411 terms. And, on a recommand of a cryptanalyst
  % they create different word for each lettre, in order to break the possible repeat of words
    \begin{itemize}
	\item The original dictionary of 211 terms has been progressively
	expanded to 411.
	\item  Multiple words to spell one letter.
	% A cryptanalyst has though it was a better idea to use
	%multiple words for one letter
	% Because of the critic of a cryptanalyst
    \end{itemize}
...
\begin{frame}
  ...
  % However, the code steal have some flaw. Indeed, Kerchhoff's principles ...
    ...
    \item<1-> Kerckhoffs's principles aren't observed
      % The secret live in the secrecy of the code
      % So, if a code talker is caught by the ennemy, he could tell him the code
	\item<2-> Non uniqueness of the translations on a message.
		% By the nature itself of the translation
		% It it inherent to the translation
		% SO IT IS NOT a cryptosystem
    \end{itemize}
	In practice:
    \begin{itemize}
...
\begin{frame}
  % To conclude , we can focus on what happened to the Navajo after the war
  \frametitle{What happen after to the Navajo}
  Vietnam War : Program declasified (1968)

  Cause : Machine faster

  Consequences : Beginning of recognition of code talkers
  ...
  \item Poor conditions life
    % live on uranium mines. In 2017, 600$ to cleanup 94 mines
    % Road impassable because different government didn't want to do pay for it
